...
{
	idle.frames.push_back({ 180, 558, 67, 147 });	
	idle.frames.push_back({ 256, 558, 67, 147 });
	idle.loop = true;
	idle.speed = 0.5f;

	// move right
	right.frames.push_back({ 870, 720, 73, 137 }); 
	right.frames.push_back({ 794, 720, 73, 137 });
	right.frames.push_back({ 698, 736, 93, 127 }); 
	right.frames.push_back({ 596, 736, 93, 127 }); 
	right.loop = false;
	right.speed = 0.1f;

	// Reverse right
	reverseRight.frames.push_back({ 596, 736, 93, 127 }); 
	reverseRight.frames.push_back({ 698, 736, 93, 127 }); 
	reverseRight.frames.push_back({ 794, 720, 73, 137 }); 
	reverseRight.frames.push_back({ 870, 720, 73, 137 }); 
	reverseRight.loop = false;
	reverseRight.speed = 0.1f;

	// End right
	endRight.frames.push_back({ 462, 752, 127, 111 });  
	endRight.frames.push_back({ 332, 752, 127, 111 }); 
	endRight.loop = true;
	endRight.speed = 0.5f;

	// Move left
	left.frames.push_back({ 334, 562, 73, 137 });  
	left.frames.push_back({ 410, 562, 73, 137 }); 
	left.frames.push_back({ 486, 578, 93, 127 }); 
	left.frames.push_back({ 588, 578, 93, 127 }); 
	left.loop = false;
	left.speed = 0.1f;

	// Reverse left
	reverseLeft.frames.push_back({ 486, 578, 93, 127 }); 
	reverseLeft.frames.push_back({ 588, 578, 93, 127 }); 
	reverseLeft.frames.push_back({ 334, 562, 73, 137 }); 
	reverseLeft.frames.push_back({ 410, 562, 73, 137 }); 
	reverseLeft.loop = false;
	reverseLeft.speed = 0.1f;

	// End left
	endLeft.frames.push_back({ 688, 594, 127, 111 });  
	endLeft.frames.push_back({ 818, 594, 127, 111 }); 
	endLeft.loop = true;
	endLeft.speed = 0.5f;

	// Falling
	falling.frames.push_back({ 13, 98, 93, 24 });  
	falling.frames.push_back({ 242, 91, 93, 24 }); 
	falling.frames.push_back({ 687, 79, 101, 27 }); 
	falling.frames.push_back({ 911, 73, 104, 29 }); 
	falling.frames.push_back({ 1130, 71, 112, 29 }); 
	falling.frames.push_back({ 30, 191, 119, 29 }); 
	falling.frames.push_back({ 257, 190, 117, 30 }); 
	falling.frames.push_back({ 477, 185, 124, 28 }); 
	falling.frames.push_back({ 697, 182, 124, 29 }); 
	falling.frames.push_back({ 917, 177, 119, 30 }); 
	falling.frames.push_back({ 1136, 175, 131, 31 }); 
	falling.frames.push_back({ 36, 294, 135, 33 }); 
	falling.loop = false;
